...
public class NSum {
    ...
    public List<int[]> twoSum1(int[] nums, int target) {
        ...
        if(nums == null || (N = nums.length) < 2) return res;
        // 先对数组排序
        Arrays.sort(nums);
        int left = 0;
        int right = N - 1;
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        // 先确定一个数字，将规模缩小为两数之和问题，然后再用双指针问题求解
        for(int a = 0; a < N - 2; a++) {
            // 去重
            ...
            while(left < right) {
                // 快速判断子数组中是否一定不存在数据对
                if(2 * nums[left] > aim || 2 * nums[right] < aim) break;
                int sum = nums[left] + nums[right];
                if(sum == aim) {
        ...
        for(int a = 0; a < N - 3; a++) {
            ...
            for(int b = a + 1; b < N - 2; b++) {
                ...
                while(left < right) {
                    ...
                    int sum = nums[left] + nums[right];
                    if(sum == aim) {
                        res.add(List.of(nums[a], nums[b], nums[left], nums[right]));
                    }
                    if(sum >= aim) {
                        // 寻找下一个right
                        int temp = right;
